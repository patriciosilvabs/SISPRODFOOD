-- Função atualizada: considera lotes em andamento antes de criar novos
...
DECLARE
  ...
  -- NOVAS VARIÁVEIS: Ajuste dinâmico
  v_unidades_em_andamento integer := 0;
  v_unidades_finalizadas integer := 0;
  v_demanda_ajustada integer;
  v_excesso_producao integer := 0;
BEGIN
  ...
  -- =====================================================
  -- NOVO: Calcular unidades já em produção (em_preparo + em_porcionamento)
  -- =====================================================
  ...
  -- NOVO: Calcular unidades já finalizadas hoje
  SELECT COALESCE(SUM(COALESCE(unidades_reais, unidades_programadas)), 0)::integer
  INTO v_unidades_finalizadas
  FROM producao_registros
  WHERE item_id = p_item_id
  ...
  -- =====================================================
  -- NOVO: Ajustar demanda considerando produção em andamento e finalizada
  -- =====================================================
  v_demanda_ajustada := GREATEST(0, v_demanda_lojas - v_unidades_em_andamento - v_unidades_finalizadas);
  
  -- Se demanda ajustada for zero, significa que já temos produção suficiente
  IF v_demanda_ajustada <= 0 THEN
    -- Calcular excesso (se houver)
    v_excesso_producao := (v_unidades_em_andamento + v_unidades_finalizadas) - v_demanda_lojas;
    
    -- Deletar lotes pendentes (já temos produção suficiente)
    ...
    RETURN jsonb_build_object(
      ...
      'unidades_finalizadas', v_unidades_finalizadas,
      'excesso_producao', GREATEST(0, v_excesso_producao),
      'dia_operacional_usado', v_hoje
    );
  END IF;
  
  -- Se não há demanda ajustada (ninguém precisa), não criar registro
  ...
  -- 5. Calcular necessidade total AJUSTADA (usando demanda ajustada + reserva)
  ...
  IF v_item_data.unidade_medida = 'lote_masseira' THEN
    ...
    -- Lotes necessários baseado na DEMANDA AJUSTADA
    ...
    -- Deletar lotes pendentes anteriores
    DELETE FROM producao_registros
    WHERE item_id = p_item_id
      AND status = 'a_produzir'
      AND organization_id = p_organization_id;
    ...
    IF v_lotes_necessarios > 1 THEN
      ...
      RETURN jsonb_build_object(
        ...
        'dia_operacional_usado', v_hoje,
        'demanda_original', v_demanda_lojas,
        'demanda_ajustada', v_demanda_ajustada,
        'unidades_em_andamento', v_unidades_em_andamento,
        'unidades_finalizadas', v_unidades_finalizadas
      );
    ELSE
