-- Função SECURITY DEFINER para criar/atualizar registros de produção
-- Esta função pode ser chamada por qualquer usuário autenticado da organização
-- e executará com permissões elevadas para criar registros em producao_registros
...
BEGIN
  ...
  -- 2. Calcular demanda total de todas as lojas (apenas contagens do dia atual)
  SELECT 
  ...
  WHERE item_porcionado_id = p_item_id
    ...
    AND updated_at >= v_hoje::timestamp
    AND updated_at < (v_hoje + interval '1 day')::timestamp
  ...
  WHERE c.item_porcionado_id = p_item_id
    ...
    AND c.updated_at >= v_hoje::timestamp
    AND c.updated_at < (v_hoje + interval '1 day')::timestamp
