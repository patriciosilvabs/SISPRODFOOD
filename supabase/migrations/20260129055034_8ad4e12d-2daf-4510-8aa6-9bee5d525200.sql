-- Corrigir a trigger function para excluir lojas tipo CPD do cálculo de demanda
...
BEGIN
  ...
  -- CORREÇÃO: Excluir lojas do tipo 'cpd' da demanda (CPD não é consumidor, é produtor)
  SELECT 
  ...
  WHERE cp.item_porcionado_id = NEW.item_porcionado_id
    ...
    AND l.tipo != 'cpd';  -- EXCLUIR CPD DA DEMANDA
  ...
  -- Se não há demanda, deletar registros pendentes existentes
  IF v_total_a_produzir <= 0 THEN
