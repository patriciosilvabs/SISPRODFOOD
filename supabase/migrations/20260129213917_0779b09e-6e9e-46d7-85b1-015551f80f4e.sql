-- Modificar função criar_ou_atualizar_producao_registro
-- Para criar cards especiais com status 'estoque_disponivel' quando o estoque CPD cobre toda a demanda
-- Isso permite que operadores confirmem visualmente a disponibilidade do estoque
...
DECLARE
    ...
    v_ultimo_id uuid;
    -- Variáveis para gatilho mínimo
    v_gatilho_minimo integer := 0;
    v_estoque_cpd integer := 0;
    v_demanda_total integer := 0;
    v_saldo_liquido integer := 0;
    -- NOVA variável para buscar CPD
    v_cpd_loja_id uuid;
    -- Variável para detalhes de lojas consolidado
    v_detalhes_lojas_consolidado jsonb := '[]'::jsonb;
BEGIN
    ...
    -- Calcular demanda total de todas as lojas para este item e data
    -- EXCLUIR lojas do tipo 'cpd' (CPD produz, não consome)
    ...
    -- CORREÇÃO: Buscar estoque CPD da contagem_porcionados.final_sobra (fonte real)
    ...
    v_saldo_liquido := v_demanda_total - v_estoque_cpd;

    -- ============================================================
    -- VERIFICAÇÃO DO GATILHO MÍNIMO
    -- ============================================================
    IF v_gatilho_minimo > 0 AND v_saldo_liquido > 0 AND v_saldo_liquido < v_gatilho_minimo THEN
        ...
        WHERE item_id = p_item_id
          ...
          AND status IN ('a_produzir', 'estoque_disponivel');
    ...
    -- Remover do backlog se existir (pois agora vai produzir ou confirmar estoque)
    ...
    -- ============================================================
    -- NOVO: ESTOQUE CPD COBRE TODA A DEMANDA (saldo <= 0)
    -- Criar card especial para confirmação visual
    -- ============================================================
    IF v_saldo_liquido <= 0 AND v_demanda_total > 0 THEN
        ...
        -- Buscar detalhes das lojas com demanda para exibição no card
        SELECT jsonb_agg(
            jsonb_build_object(
                'loja_id', cp.loja_id,
                'loja_nome', l.nome,
        ...
        INTO v_detalhes_lojas_consolidado
        FROM contagem_porcionados cp
        JOIN lojas l ON l.id = cp.loja_id
        WHERE cp.item_porcionado_id = p_item_id
          AND cp.organization_id = p_organization_id
        ...
        -- Criar card especial de "Estoque Disponível"
        INSERT INTO producao_registros (
            ...
            COALESCE(v_detalhes_lojas_consolidado, '[]'::jsonb),
            v_demanda_total,
            v_reserva_dia,
            p_organization_id,
            v_usuario_id,
    ...
    -- Se demanda é zero, apenas limpar cards existentes
    ...
    -- Deletar registros existentes para este item/data que estão "a_produzir" ou "estoque_disponivel"
    DELETE FROM producao_registros
    WHERE item_id = p_item_id
      AND organization_id = p_organization_id
      AND data_referencia = v_data_hoje
    ...
    -- ============================================================
    -- LÓGICA NORMAL: Loop por CADA LOJA com demanda > 0 (excluindo CPD)
    -- ============================================================
    FOR v_contagem IN 
        SELECT 
            cp.loja_id,
