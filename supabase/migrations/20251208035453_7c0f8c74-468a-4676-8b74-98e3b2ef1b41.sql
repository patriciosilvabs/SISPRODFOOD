-- 1. Atualizar função get_current_date para usar timezone do Brasil
...
-- 2. Atualizar função criar_ou_atualizar_producao_registro para usar timezone do Brasil
...
DECLARE
  ...
  v_dia_semana text;
  -- CORREÇÃO: Usar timezone do Brasil para determinar "hoje"
...
BEGIN
  ...
  -- 2. Calcular demanda total de todas as lojas (apenas contagens do dia atual NO HORÁRIO DO BRASIL)
  -- CORREÇÃO: Converter updated_at para timezone do Brasil antes de comparar com v_hoje
  SELECT 
  ...
  WHERE item_porcionado_id = p_item_id
    ...
    AND (updated_at AT TIME ZONE 'America/Sao_Paulo')::date = v_hoje
  ...
  -- 3. Determinar dia da semana de amanhã (também no timezone do Brasil)
  ...
  -- 6. Construir detalhes por loja (CORREÇÃO: usar timezone do Brasil)
  SELECT jsonb_agg(
    jsonb_build_object(
      'loja_id', c.loja_id,
      'loja_nome', l.nome,
  ...
  WHERE c.item_porcionado_id = p_item_id
    ...
    AND (c.updated_at AT TIME ZONE 'America/Sao_Paulo')::date = v_hoje
  ...
  IF v_item_data.usa_traco_massa = true AND v_item_data.unidade_medida = 'traco' AND v_item_data.equivalencia_traco IS NOT NULL THEN
    -- FILA DE TRAÇOS: Deletar registros antigos e criar N novos
