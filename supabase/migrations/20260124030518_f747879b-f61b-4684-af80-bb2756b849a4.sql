...
-- Corrigir função para não inserir producao_lote_id (que tem FK para producao_lotes)
-- Apenas lote_producao_id é usado para agrupar traços
...
BEGIN
    ...
    -- Calcular número de lotes para itens com traço de massa
    IF v_item.usa_traco_massa = true AND COALESCE(v_item.quantidade_por_lote, 0) > 0 THEN
        v_num_lotes := CEIL(v_demanda_total::numeric / v_item.quantidade_por_lote);
