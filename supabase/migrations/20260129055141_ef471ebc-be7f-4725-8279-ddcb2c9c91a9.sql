-- Corrigir a função RPC principal para excluir lojas tipo CPD do cálculo de demanda
...
BEGIN
    ...
    -- CORREÇÃO: Excluir lojas do tipo 'cpd' (CPD produz, não consome)
    ...
    FOR v_contagem IN 
        ...
        WHERE cp.item_porcionado_id = p_item_id
          ...
          AND cp.dia_operacional = v_data_hoje
          AND cp.a_produzir > 0
          AND l.tipo != 'cpd'  -- EXCLUIR CPD DO LOOP
