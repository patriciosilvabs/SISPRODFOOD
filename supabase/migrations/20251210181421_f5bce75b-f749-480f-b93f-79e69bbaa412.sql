...
-- Atualizar função para criar fila sequencial de lotes para LOTE_MASSEIRA
...
BEGIN
  ...
  IF v_item_data.unidade_medida = 'lote_masseira' THEN
    ...
    -- ========================================
    -- LOTE_MASSEIRA: FILA SEQUENCIAL DE LOTES
    -- ========================================
    IF v_lotes_necessarios > 1 THEN
      -- Criar múltiplos registros em fila (igual traço/lote)
      v_lote_id := gen_random_uuid();
      
      -- Deletar registros anteriores pendentes do mesmo item
      ...
      FOR v_seq IN 1..v_lotes_necessarios LOOP
        INSERT INTO producao_registros (
          ...
          v_item_data.massa_gerada_por_lote_kg,         -- peso por lote individual
          ...
          p_organization_id,
          v_hoje,
          1,                                            -- cada card é 1 lote individual
          v_item_data.farinha_por_lote_kg,              -- farinha por lote individual
          v_item_data.massa_gerada_por_lote_kg,         -- massa por lote individual
          v_seq,                                        -- sequência: 1, 2, 3...
          v_lote_id,                                    -- agrupa todos os lotes
          v_seq > 1,                                    -- bloqueado se não é o primeiro
          'aguardando'
        );
      END LOOP;
    ...
    ELSE
      -- Apenas 1 lote: criar/atualizar registro único
