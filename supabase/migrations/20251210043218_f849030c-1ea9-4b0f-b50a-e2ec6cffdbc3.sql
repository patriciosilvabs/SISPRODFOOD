-- Atualizar função criar_ou_atualizar_producao_registro para aceitar 'lote' além de 'traco'
...
BEGIN
  ...
  -- 1. Buscar dados do item (INCLUINDO perda_percentual_adicional)
  SELECT 
  ...
  -- 7. Calcular unidades programadas e peso (COM PERDA ADICIONAL)
  -- CORRIGIDO: Aceitar tanto 'traco' quanto 'lote'
  v_perda_adicional := COALESCE(v_item_data.perda_percentual_adicional, 0);
  
  IF v_item_data.unidade_medida IN ('traco', 'lote') AND v_item_data.equivalencia_traco IS NOT NULL AND v_item_data.consumo_por_traco_g IS NOT NULL THEN
