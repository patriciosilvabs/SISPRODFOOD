-- Atualizar função RPC com parâmetro p_is_incremental
...
BEGIN
  ...
  -- ========== NOVO: SE p_is_incremental = true OU tem produção ativa, criar INCREMENTAL ==========
  IF p_is_incremental OR v_tem_producao_ativa THEN
      ...
      IF v_necessidade_total > v_total_programado THEN
        ...
        FOR v_seq IN v_seq_inicial..(v_seq_inicial + v_lotes_adicionais - 1) LOOP
          INSERT INTO producao_registros (
            ...
            v_lote_id,
            true, -- bloqueado (precisa esperar os anteriores)
            'aguardando',
            v_hoje,
            true
          );
        ...
        RETURN jsonb_build_object(
          ...
          'is_incremental_param', p_is_incremental,
    ...
    -- ===== LÓGICA PARA LOTE_MASSEIRA E OUTROS (INCREMENTAL) =====
  ...
  -- ========== SCENARIO: SEM PRODUÇÃO ATIVA E NÃO É INCREMENTAL - SUBSTITUIR LIVREMENTE ==========
  
  -- If no demand and no reserve, no need to create
  IF v_demanda_atual = 0 AND v_reserva_dia = 0 THEN
    -- Limpar registros a_produzir existentes (de qualquer data)
  ...
  IF v_item_data.unidade_medida = 'lote_masseira' THEN
    ...
    -- SUBSTITUIR: Delete existing a_produzir batches (DE QUALQUER DATA)
    DELETE FROM producao_registros
    WHERE item_id = p_item_id
      AND status = 'a_produzir'
      AND organization_id = p_organization_id;
  ...
  IF v_item_data.usa_traco_massa = true AND v_item_data.unidade_medida IN ('traco', 'lote') AND v_item_data.equivalencia_traco IS NOT NULL THEN
    ...
    -- SUBSTITUIR: Delete existing a_produzir (DE QUALQUER DATA)
    DELETE FROM producao_registros
    WHERE item_id = p_item_id
      AND status = 'a_produzir'
      AND organization_id = p_organization_id;
  ...
  ELSE
    -- Simple batch creation - SUBSTITUIR (DE QUALQUER DATA)
    DELETE FROM producao_registros
    WHERE item_id = p_item_id
      AND status = 'a_produzir'
      AND organization_id = p_organization_id;
