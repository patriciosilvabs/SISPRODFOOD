-- Atualizar função criar_ou_atualizar_producao_registro para suportar LOTE_MASSEIRA
...
BEGIN
  ...
  IF v_item_data.unidade_medida = 'lote_masseira' THEN
    ...
    -- Peso médio operacional (usar configurado ou calcular da faixa)
    v_peso_medio_g := COALESCE(
      v_item_data.peso_medio_operacional_bolinha_g,
      v_item_data.peso_alvo_bolinha_g,
      (COALESCE(v_item_data.peso_minimo_bolinha_g, 0) + COALESCE(v_item_data.peso_maximo_bolinha_g, 0)) / 2
    );
    
    IF v_peso_medio_g <= 0 THEN
      RETURN jsonb_build_object('success', false, 'error', 'Peso médio operacional não definido');
    END IF;
